    ...
    >
      <div className="flex flex-col items-center gap-6 animate-fade-in">
        <h1 className="text-5xl font-extrabold tracking-tight text-gray-100 sm:text-6xl md:text-7xl">
          {t('startTitle').split(',').map((part, index) => 
              index === 1 ? <span key={index} className="text-blue-400">{part}</span> : part
        ...
        <div className="mt-16 w-full">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="bg-black/20 p-6 rounded-lg border border-gray-700/50 flex flex-col items-center text-center">
                    ...
                    <h3 className="text-xl font-bold text-gray-100">{t('featureRetouchTitle')}</h3>
                    <p className="mt-2 text-gray-400">{t('featureRetouchDescription')}</p>
                </div>
                <div className="bg-black/20 p-6 rounded-lg border border-gray-700/50 flex flex-col items-center text-center">
                    <div className="flex items-center justify-center w-12 h-12 bg-gray-700 rounded-full mb-4">
                       <PaletteIcon className="w-6 h-6 text-blue-400" />
                    </div>
                    <h3 className="text-xl font-bold text-gray-100">{t('featureFilterTitle')}</h3>
                    <p className="mt-2 text-gray-400">{t('featureFilterDescription')}</p>
                </div>
                <div className="bg-black/20 p-6 rounded-lg border border-gray-700/50 flex flex-col items-center text-center">
                    <div className="flex items-center justify-center w-12 h-12 bg-gray-700 rounded-full mb-4">
                       <SunIcon className="w-6 h-6 text-blue-400" />
                    </div>
                    <h3 className="text-xl font-bold text-gray-100">{t('featureAdjustTitle')}</h3>
                    <p className="mt-2 text-gray-400">{t('featureAdjustDescription')}</p>
                </div>
            </div>
        </div>
