...
const CropPanel: React.FC<CropPanelProps> = ({ onApplyCrop, onSetAspect, isLoading, isCropping }) => {
  ...
  const aspects: { name: AspectRatio, value: number | undefined, label: string }[] = [
    { name: 'free', value: undefined, label: t('free') },
  ...
  return (
    <div className="w-full bg-gray-800/50 border border-gray-700 rounded-lg p-4 flex flex-col items-center gap-4 animate-fade-in backdrop-blur-sm">
      <h3 className="text-lg font-semibold text-gray-300">{t('cropImage')}</h3>
      <p className="text-sm text-gray-400 -mt-2">{t('cropImageDescription')}</p>
